...
<xsl:template match="Interface[descriptive]">
    ...
    <div class="interface" id="{@id}">
        <h3><xsl:value-of select="concat('2.',1+count(preceding::Interface))"/>. <xsl:value-of select="@name"/></h3>
        <xsl:apply-templates select="descriptive/brief"/>
        <xsl:apply-templates select="webidl"/>
...
<!-- start Kevin Smith insert 20 Oct 2010
 ...
 -->
<xsl:key name="reference-link-key" match="Interface | Typedef" use="@name"/>
...
<xsl:template match="ref">
    ...
    <xsl:when test="$reference-clean='Event'">
      <a href="http://www.w3.org/TR/2007/WD-DOM-Level-3-Events-20071221/events.html#Events-Event">
                    <xsl:value-of select="$reference-clean"/>
      </a>
    </xsl:when>
